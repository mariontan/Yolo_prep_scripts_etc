...
#        self.delBtn.grid(row = 6, column = 2, sticky = W+E+N)
        ...
        self.frontBtn = Button(self.ctrPanel, text = 'Front', command = self.frontImage)
...
#                print(lines)
                for row in lines:
                    line = [float(w) for w in row.split(' ')]
#                    print('line')
#                    print(line)
    ...
    def saveImage(self):
        ...
        for i in range(len(self.bboxList)):
            ...
            w = (box[2] - box[0]) / imgw
            h = (box[3] - box[1]) / imgh
            labels.append(str(self.imgclass)+' '+str(x)+' '+str(y)+' '+str(w)+' '+str(h)+'\n')
    ...
    def saveList(self):
        lsts = []
        lsts.append([self.frontLst])
        lsts.append([self.rearLst])
        lsts.append([self.rightLst])
        lsts.append([self.leftLst])
        lsts.append([self.unknownLst])
        name = ['front','rear','right','left','unknown']    
        i = 0
        for lst in lsts:
            df = pd.Series(lst[0])
    ...
    def frontImage(self, event = None):
        self.saveImage()
        self.frontLst.append(self.imagename)
        print('front:',self.imagename)
        if self.cur < self.total:
            self.cur += 1
            self.loadImage()
            
    def rearImage(self, event = None):
        self.saveImage()
        self.rearLst.append(self.imagename)
        print('rear:',self.imagename)
        if self.cur < self.total:
            self.cur += 1
            self.loadImage()
    
    def rightImage(self, event = None):
        self.saveImage()
        self.rightLst.append(self.imagename)
        print('right:', self.imagename)
        if self.cur < self.total:
            self.cur += 1
            self.loadImage()
    
    def leftImage(self, event = None):
        self.saveImage()
        self.leftLst.append(self.imagename)
        print('left:', self.imagename)
        if self.cur < self.total:
            self.cur += 1
            self.loadImage()
            
    def unknownImage(self, event = None):
        self.saveImage()
        self.unknownLst.append(self.imagename)
        print('unknown:', self.imagename)
        if self.cur < self.total:
            self.cur += 1
            self.loadImage()
